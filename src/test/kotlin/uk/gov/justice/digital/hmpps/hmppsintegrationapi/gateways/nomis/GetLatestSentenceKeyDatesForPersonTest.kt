...
import uk.gov.justice.digital.hmpps.hmppsintegrationapi.models.prisonApi.PrisonApiSentenceKeyDates
      ...
      it("can use the RestApiClient") {
        ...
        whenever(apiClient.get(eq(sentencePath), eq(PrisonApiOffenderSentence::class), eq(headers), isNull())).thenReturn(
          RestApiResponse(
            "Test",
            HttpStatus.OK,
            PrisonApiOffenderSentence(
              PrisonApiSentenceKeyDates(
                automaticReleaseDate = LocalDate.parse("2027-02-01"),
              ),
            ),
          ),
        )
        ...
        // Then
        response.data?.actualParoleDate?.shouldBe(LocalDate.parse("2027-02-01"))
      }
    },
  )
