...
class AuthorisationFilterTest {
  ...
  private val mockOboService = mock(OboService::class.java)
  ...
  fun fullMockFilterChain(
    ...
    mockFilterChain(
      AuthorisationFilter(authService, mockTelemetryService, mockRoleService, mockOboService),
      finalFilter,
    )
  ...
  fun `calls the onward chain when path found in roles (but not in includes)`() {
    ...
    val authorisationFilter = AuthorisationFilter(authorisationService, mockTelemetryService, mockRoleService, mockOboService)
    val finalFilter = mock(Filter::class.java)
  ...
  fun `returns error when the roles or consumer includes do not contain the path`() {
    ...
    val chain = mockFilterChain(AuthorisationFilter(authService, mockTelemetryService, mockRoleService, mockOboService))
