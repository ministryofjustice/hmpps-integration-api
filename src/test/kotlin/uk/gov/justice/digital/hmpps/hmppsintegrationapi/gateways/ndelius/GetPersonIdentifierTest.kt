...
class GetPersonIdentifierTest(
  ...
  val nDeliusGateway: NDeliusGateway,
) : DescribeSpec(
    {
      ...
      val path = "/identifier-converter/noms-to-crn/$nomisNumber"
      ...
      beforeEach {
        ...
        nDeliusApiMockServer.stubForGet(
          path,
          File(
            "src/test/kotlin/uk/gov/justice/digital/hmpps/hmppsintegrationapi/gateways/ndelius/fixtures/GetPersonIdentifierResponse.json",
          ).readText(),
        )
      ...
      it("authenticates using HMPPS Auth with credentials") {
        nDeliusGateway.getPersonIdentifier(nomisNumber)

        verify(hmppsAuthGateway, VerificationModeFactory.times(1)).getClientToken("nDelius")
      }

      it("returns CRN with matching nomis number") {
        val response = nDeliusGateway.getPersonIdentifier(nomisNumber)

        response?.crn.shouldBe(deliusCrn)
        response?.nomsId.shouldBe(nomisNumber)
      }
