  ...
  private val getAddressesForPersonService: GetAddressesForPersonService,
) : DescribeSpec(
    {
      ...
      it("Person service error → Return person service error") {
        ...
        whenever(personService.getNomisNumberWithFiltering(hmppsId, filters)).thenReturn(
          Response(
            data = null,
            errors = errors,
          ),
      ...
      it("Nomis number, Delius success, Nomis success → Merge responses ") {
        whenever(personService.getNomisNumberWithFiltering(hmppsId, filters)).thenReturn(Response(NomisNumber(nomisNumber)))
      ...
      it("No nomis number, delius success → return Delius") {
        whenever(personService.getNomisNumberWithFiltering(hmppsId, filters)).thenReturn(Response(NomisNumber(null)))
        whenever(deliusGateway.getAddressesForPerson(hmppsId)).thenReturn(Response(listOf(deliusAddress)))
