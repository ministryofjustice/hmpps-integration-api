...
class GetVisitRestrictionsForPersonServiceTest(
  ...
  private val getVisitRestrictionsForPersonService: GetVisitRestrictionsForPersonService,
    ...
    beforeEach {
      ...
      whenever(getPersonService.getNomisNumberWithFiltering(hmppsId = hmppsId, filters)).thenReturn(
    ...
    it("returns an error when getNomisNumberWithFiltering returns an error") {
      whenever(getPersonService.getNomisNumberWithFiltering(hmppsId = hmppsId, filters)).thenReturn(
        Response(
          data = null,
          errors =
