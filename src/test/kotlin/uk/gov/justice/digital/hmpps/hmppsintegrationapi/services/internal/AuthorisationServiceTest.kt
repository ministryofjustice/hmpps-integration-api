...
class AuthorisationServiceTest : ConfigTest() {
  ...
  val authorisationService =
    AuthorisationService(
      AuthorisationConfig(mapOf("consumer-name" to ConsumerConfig(include = null, filters = ConsumerFilters(prisons = listOf("MDI")), roles = listOf("test-role"))), roles = mapOf("test-role" to testRole)),
    )

  @DisplayName("doesConsumerHaveIncludesAccess")
  @Nested
  inner class TestConsumerHaveIncludesAccess {
    val consumerConfig = ConsumerConfig(listOf("/persons/.*"), ConsumerFilters(null, null), listOf())

    @Test
    fun `access is allowed when the path is listed under that consumer`() {
      val authResult =
        authorisationService.doesConsumerHaveIncludesAccess(
          consumerConfig,
          requestedPath,
        )
    ...
    @Test
    fun `access is denied when the extracted consumer is null`() {
      val result = authorisationService.doesConsumerHaveIncludesAccess(null, "")
      result.shouldBeFalse()
    }

    @Test
    fun `when the path isn't listed as allowed on the consumer`() {
      val invalidPath = "/some-other-path/123"
      val result = authorisationService.doesConsumerHaveIncludesAccess(consumerConfig, invalidPath)
      result.shouldBeFalse()
    }
  }
  ...
  @DisplayName("doesConsumerHaveIncludesAccess")
  @Nested
  inner class TestDoesConsumerHaveRoleAccess {
    val consumerRolesInclude = listOf("/persons/.*")

    @Test
    fun `access is allowed when the path is listed in the role included paths`() {
      val authResult =
        authorisationService.doesConsumerHaveRoleAccess(
    ...
    @Test
    fun `access is denied when the role has no included paths`() {
      val result = authorisationService.doesConsumerHaveRoleAccess(emptyList(), requestedPath)
      result.shouldBeFalse()
    }
    ...
    fun `when the path isn't listed as allowed on the consumer`() {
      val invalidPath = "/some-other-path/123"
      val result = authorisationService.doesConsumerHaveRoleAccess(consumerRolesInclude, invalidPath)
      result.shouldBeFalse()
    }
  }
  ...
  @Nested
  inner class TestAuthorised {
    @Test
    fun `access is allowed when the path is listed in the role included paths`() {
      val authorisationConfig =
      ...
      val authorisationService = AuthorisationService(authorisationConfig)
      val authResult =
        authorisationService.authorised(
          "tester",
          requestedPath,
        )
      authResult.shouldBeTrue()
    ...
    @Test
    fun `access is allowed when the path is listed in the included paths and not in the roles`() {
      val authorisationService =
  ...
  @DisplayName("verifyNormalisedPathParameters")
  @Nested
  inner class TestVerifyNormalisedPathParameters {
    @Test
    fun `works with normalised path parameters`() {
      authorisationService.matches("/v1/persons/123", "/v1/persons/{hmppsId}") shouldBe true
    }
  }
  ...
  fun `no supervision status filters in role`() {
    ...
    val filters = AuthorisationService(AuthorisationConfig(mapOf("consumer-name" to ConsumerConfig(include = null, filters = ConsumerFilters(prisons = listOf("MDI")), roles = listOf("test-role"))), roles = mapOf("test-role" to testRole))).allFilters(consumer, listOf(role))
