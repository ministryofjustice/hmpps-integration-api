...
class GetDeallocationReasonsGatewayTest(
  ...
  private val activitiesGateway: ActivitiesGateway,
) : DescribeSpec(
    {
      ...
      it("authenticates using HMPPS Auth with credentials") {
        activitiesGateway.getDeallocationReasons()
      ...
      it("Returns attendance reasons") {
        mockServer.stubForGet(
          path,
          File("src/test/kotlin/uk/gov/justice/digital/hmpps/hmppsintegrationapi/gateways/activities/fixtures/GetDeallocationReasons.json").readText(),
        )

        val result = activitiesGateway.getDeallocationReasons()
        result.errors.shouldBeEmpty()
        result.data.shouldNotBeNull()
        result.data[0].code.shouldBe("RELEASED")
