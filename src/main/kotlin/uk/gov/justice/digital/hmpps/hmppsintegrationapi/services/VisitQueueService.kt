...
class VisitQueueService(
  ...
  fun sendCreateVisit(
    ...
    val personResponse = getPersonService.getNomisNumberWithFiltering(hmppsId = visitPrisonerId, filters = filters)
    ...
    val contacts =
      buildList {
        while (!isLastPage) {
          ...
          isLastPage = response.data?.toPaginatedPrisonerContacts()?.isLastPage ?: true
          page++
        }
      }
