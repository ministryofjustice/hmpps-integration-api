...
class ProbationOffenderSearchGateway(@Value("\${services.probation-offender-search.base-url}") baseUrl: String) {
  ...
  fun getPerson(pncId: String): Response<Person?> {
    return try {
      val offenders = webClient.requestList<Offender>(
        ...
        mapOf("pncNumber" to pncId, "isValid" to true),
      )
  ...
  fun getPersons(firstName: String?, surname: String?, searchWithinAliases: Boolean = false): Response<List<Person>> {
    val requestBody = mapOf("firstName" to firstName, "surname" to surname, "includeAliases" to searchWithinAliases, "isValid" to true)
  ...
  fun getAddressesForPerson(pncId: String): Response<List<Address>> {
    val requestBody = mapOf("pncNumber" to pncId, "isValid" to true)
